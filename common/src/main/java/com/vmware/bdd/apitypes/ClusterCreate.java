...
public class ClusterCreate implements Serializable {
			...
			if (storageDef != null) {
				...
				if (storageType != null) {
					...
					if (!storageType.equals(DatastoreType.TEMPFS.toString())
							&& !storageType.equals(DatastoreType.LOCAL
									.toString())
							&& !storageType.equals(DatastoreType.SHARED
									.toString())) {
						failedMsgList.add("Invalid storage type " + storageType
								+ ". " + Constants.STORAGE_TYPE_ALLOWED);
					} else if (storageType.equals(DatastoreType.TEMPFS
							.toString())) {// tempfs disk type
					// if (nodeGroupCreate.getRoles().contains(
					// HadoopRole.HADOOP_TASKTRACKER.toString())) {//compute
					// node
					...
					// }
						...
						// TEMPFS is a temporary feature, so comment off it to
		...
		if (memCap != null) {
			if (memCap <= 0) {
				failedMsgList.add(new StringBuilder()
						.append(nodeGroup.getName()).append(".")
						.append("memCapacityMB=").append(memCap).append(".")
						.toString());
			} else {
	...
	private void makeVmMemoryDivisibleBy4(NodeGroupCreate nodeGroup,
		...
		if (memoryCap > 0) {
			// VM's memory must be divisible by 4, otherwise VM can not be
			// started
	...
	public boolean supportedWithHdfs2() {
		if (this.getDistroVendor().equalsIgnoreCase(Constants.APACHE_VENDOR)
				|| this.getDistroVendor().equalsIgnoreCase(
						Constants.MAPR_VENDOR)
				|| this.getDistroVendor().equalsIgnoreCase(
						Constants.GPHD_VENDOR)) {
			return false;
		}
		return true;
	}
	...
	public void validateCDHVersion(List<String> warningMsgList) {
		// If current distro's version is greater than cdh4.2.1, the FQDN must
		// be configured.
		if (this.getDistroVendor().equalsIgnoreCase(Constants.CDH_VENDOR)) {
			...
			String version = this.getDistroVersion().substring(matcher.start(),
					matcher.end());
		...
		if (nodeGroups != null && nodeGroups.length > 0) {
			...
			for (NodeGroupCreate nodeGroup : nodeGroups) {
				if (CommonUtil.isBlank(nodeGroup.getName())
						|| !CommonUtil.validateNodeGroupName(nodeGroup
								.getName())) {
					invalidNodeGroupNames.append("'")
							.append(nodeGroup.getName()).append("'")
							.append(",");
				}
			}
			if (invalidNodeGroupNames.length() > 0) {
				invalidNodeGroupNames.delete(
						invalidNodeGroupNames.length() - 1,
						invalidNodeGroupNames.length());
				throw ClusterConfigException
						.NODE_GROUP_NAME_IS_INVALID(invalidNodeGroupNames
								.toString());
			}
		} else {
	...
	public void verifyClusterNameLength() {
		...
		int rpNeedLength = MAX_VC_OBJECT_NAME_LENGTH
				- ConfigInfo.getSerengetiUUID().length()
				- RP_LINK_SYMBOL_LENGTH;
		if (rpNeedLength <= 0) {
			throw ClusterConfigException
					.UUID_TOO_LONG(MAX_VC_OBJECT_NAME_LENGTH
							- RP_LINK_SYMBOL_LENGTH);
		}
		int vmNeedLength = MAX_VC_OBJECT_NAME_LENGTH - MAX_VM_INDEX_LENGTH
		...
		int clusterNameMaxLength = vmNeedLength - rpNeedLength > 0 ? rpNeedLength
				: vmNeedLength;
