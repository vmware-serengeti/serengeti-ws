...
public class AppConfigValidationFactory {
   ...
   static final Logger logger = Logger
         .getLogger(AppConfigValidationFactory.class);
   ...
   public static ValidateResult blackListHandle(Map<String, Object> config) {
      ...
      for (Entry<String, Object> configType : config.entrySet()) {
         if (((String) configType.getKey()).trim().equalsIgnoreCase("hadoop")) {
            ...
            for (Entry<String, Object> configFileEntry : propertyConfig
                  .entrySet()) {
               configFileName = configFileEntry.getKey();
               if (validateBySameFileName(configFileName,
                     configFileEntry.getValue(), blackList, validateResult,
      ...
      for (Entry<String, Object> configType : config.entrySet()) {
         if (((String) configType.getKey()).trim().equalsIgnoreCase("hadoop")) {
            ...
            for (Entry<String, Object> configFileEntry : propertyConfig
               ...
               configFileName = configFileEntry.getKey();
               if (validateBySameFileName(configFileName,
                     configFileEntry.getValue(), whiteList, validateResult,
   ...
   @SuppressWarnings("unchecked")
   private static <T> boolean validateBySameFileName(String fileName,
         Object configProperties, List<Map<String, T>> warnPropertyList,
         ValidateResult validateResult, ValidationType validationType) {
      boolean warning = false;
      for (Map<String, T> warnPropertyFileMap : warnPropertyList) {
         if (warnPropertyFileMap.containsKey(fileName)
               && configProperties instanceof Map) {
            Map<String, Object> configPropertyMap =
                  (Map<String, Object>) configProperties;
            for (Entry<String, Object> configProperty : configPropertyMap
                  .entrySet()) {
               if (validationType == ValidationType.WHITE_LIST) {
                  for (Entry<String, T> warnPropertyFileEntry : warnPropertyFileMap
                        .entrySet()) {
                     if (warnPropertyFileEntry.getValue() instanceof List) {
                        List<Object> propertyList =
                              (List<Object>) warnPropertyFileEntry.getValue();
                        if (!validateWhiteListPropertis(propertyList,
                              configProperty.getKey(),
                              String.valueOf(configProperty.getValue()),
                              validateResult)) {
                           warning = true;
                        }
                     }
                  ...
                  for (Entry<String, T> warnPropertyFileEntry : warnPropertyFileMap
                     ...
                     if (warnPropertyFileEntry.getValue() instanceof List) {
                        List<String> propertyList =
                              (List<String>) warnPropertyFileEntry.getValue();
                        for (String propertyName : propertyList) {
                           if (configProperty.getKey().equals(propertyName)) {
                              warning = true;
                              validateResult
                                    .setType(ValidateResult.Type.NAME_IN_BLACK_LIST);
                              validateResult.addFailureName(configProperty
                                    .getKey());
                           }
                        }
                     }
   ...
   private static boolean validatePropertyValueFormat(final String value,
         final String format) {
      return true;
   }

   @SuppressWarnings("unchecked")
   private static boolean validateWhiteListPropertis(List<Object> propertyList,
         String configPropertyName, String configPropertyValue,
         ValidateResult validateResult) {
      ValidateResult.Type validateType =
            ValidateResult.Type.WHITE_LIST_INVALID_NAME;
      for (Object obj : propertyList) {
         if (obj instanceof Map) {
            Map<String, String> property = (Map<String, String>) obj;

            if (property.get("name").trim()
                  .equalsIgnoreCase(configPropertyName)) {
               if (!property.get("valueFormat").isEmpty()
                     && !validatePropertyValueFormat(configPropertyValue,
                           property.get("valueFormat"))) {
      ...
      if (validateType == ValidateResult.Type.WHITE_LIST_INVALID_NAME) {
         ...
         if (validateResult.getType() == ValidateResult.Type.WHITE_LIST_INVALID_VALUE
            ...
            validateResult
                  .setType(ValidateResult.Type.WHITE_LIST_INVALID_NAME_VALUE);
         } else {
   ...
   private static String readJsonFile(final String fileName) {
      ...
      URL fileURL =
            AppConfigValidationUtils.class.getClassLoader().getResource(
                  fileName);
      if (fileURL != null) {
         InputStream in = null;
         try {
