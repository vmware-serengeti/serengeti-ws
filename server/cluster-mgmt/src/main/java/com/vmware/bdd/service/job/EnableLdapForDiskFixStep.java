...
public class EnableLdapForDiskFixStep extends TrackableTasklet {
   private static final Logger logger = Logger.getLogger(EnableLdapForDiskFixStep.class);
   ...
   public RepeatStatus executeStep(ChunkContext chunkContext, JobExecutionStatusHolder jobExecutionStatusHolder) {
      logger.info("Enable LDAP for disk fix");
      ...
      if (nodeEntity == null) {
         throw TaskException.EXECUTION_FAILED("No fixed node need to enable password.");
      }
