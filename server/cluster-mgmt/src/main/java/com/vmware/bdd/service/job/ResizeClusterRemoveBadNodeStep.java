...
public class ResizeClusterRemoveBadNodeStep extends TrackableTasklet {
   ...
   public RepeatStatus executeStep(ChunkContext chunkContext,
      ...
      List<BaseNode> existingNodes =
            JobUtils.getExistingNodes(clusterSpec, getClusterEntityMgr());
      List<BaseNode> deletedNodes = new ArrayList<BaseNode>();
      removeExcessiveOrWrongStatusNodes(existingNodes, deletedNodes, groupName,
            newInstanceNum, oldInstanceNum);
      JobUtils.removeNonExistNodes(existingNodes, deletedNodes);
      verifyExistingSuccessNodes(existingNodes, groupName, oldInstanceNum,
            clusterSpec);
      ...
      boolean deleted =
            clusteringService.syncDeleteVMs(deletedNodes, statusUpdator);
   ...
   /**
    * This method will verify placement policy for existing nodes in old defined
    * instance scope. If they violate the placement policy defined in cluster
    * spec, we don't allow resize operation
    */
   private void verifyExistingSuccessNodes(List<BaseNode> existingNodes,
      ...
      List<BaseNode> badNodes =
            clusteringService.getBadNodes(clusterSpec, needToBeVerified);
   ...
   private void removeExcessiveOrWrongStatusNodes(List<BaseNode> existingNodes,
         List<BaseNode> deletedNodes, String groupName, long newInstanceNum,
         long oldInstanceNum) {
      for (BaseNode node : existingNodes) {
         if (node.getGroupName().equals(groupName)) {
            long index = JobUtils.getVmIndex(node.getVmName());
            ...
            if (index >= oldInstanceNum) {
               ...
               VcVirtualMachine vm =
                     VcCache.getIgnoreMissing(node.getVmMobId());
               if (vm == null) {
                  deletedNodes.add(node);
                  continue;
               }
               if (!vm.isPoweredOn()
                  ...
                  VcSnapshot snapshot =
                        vm.getSnapshotByName(Constants.ROOT_SNAPSTHOT_NAME);
                  if (snapshot == null) {
