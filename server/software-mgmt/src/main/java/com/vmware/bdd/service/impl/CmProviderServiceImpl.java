...
import com.vmware.bdd.apitypes.CmMgmtServiceType;
import com.vmware.bdd.apitypes.CmNodeDef;
import com.vmware.bdd.apitypes.CmRoleDef;
import com.vmware.bdd.apitypes.CmServiceDef;
import com.vmware.bdd.apitypes.CmServiceSpec;
import com.vmware.bdd.apitypes.CmServiceType;
...
public class CmProviderServiceImpl implements ICmProviderService {
   ...
   private void provisionManagement(final CmClusterDef cluster) {
      ...
      if (cmsProvisionRequired) {
         final ApiHostRef apiHostRef = new ApiHostRef("123123141231"); // CM server hostId
    ...
    */
   private void provisionParcels(final CmClusterDef cluster) throws InterruptedException, IOException {
      ...
      for (CmServiceDef serviceDef : cluster.getServices()) {
         CmServiceType serviceType = CmServiceType.valueOf(serviceDef.getType());
   ...
   private void configureServices(final CmClusterDef cluster) throws Exception {
      ...
      for (CmServiceDef serviceDef : cluster.getServices()) {
         CmServiceType type = CmServiceType.valueOf(serviceDef.getType());
         ...
         switch (type) {
            ...
            case SOLR:
               ...
               break;
            case SOLR_INDEXER:
               apiServiceConfig.add(new ApiConfig("hbase_service", cluster.serviceNameOfType(CmServiceType.HBASE)));
               apiServiceConfig.add(new ApiConfig("solr_service", cluster.serviceNameOfType(CmServiceType.SOLR)));
               break;
            case HUE:
               apiServiceConfig.add(new ApiConfig("hue_webhdfs", cluster.serviceNameOfType(CmServiceType.HDFS_HTTP_FS)));
               apiServiceConfig.add(new ApiConfig("oozie_service", cluster.serviceNameOfType(CmServiceType.OOZIE)));
               apiServiceConfig.add(new ApiConfig("hive_service", cluster.serviceNameOfType(CmServiceType.HIVE)));
               if (serviceTypes.contains(CmServiceType.HBASE)) {
                  apiServiceConfig.add(new ApiConfig("hbase_service", cluster.serviceNameOfType(CmServiceType.HBASE)));
               }
               if (serviceTypes.contains(CmServiceType.IMPALA)) {
                  apiServiceConfig.add(new ApiConfig("impala_service", cluster.serviceNameOfType(CmServiceType.IMPALA)));
               }
               if (serviceTypes.contains(CmServiceType.SOLR)) {
                  apiServiceConfig.add(new ApiConfig("solr_service", cluster.serviceNameOfType(CmServiceType.SOLR)));
               }
               if (serviceTypes.contains(CmServiceType.SQOOP)) {
                  apiServiceConfig.add(new ApiConfig("sqoop_service", cluster.serviceNameOfType(CmServiceType.SQOOP)));
               }
               if (serviceTypes.contains(CmServiceType.HBASE_THRIFT_SERVER)) {
                  apiServiceConfig.add(new ApiConfig("hue_hbase_thrift", cluster
                        .serviceNameOfType(CmServiceType.HBASE_THRIFT_SERVER)));
               }
               break;
            case SQOOP:
               apiServiceConfig.add(new ApiConfig("mapreduce_yarn_service", serviceTypes
                     .contains(CmServiceType.YARN) ? cluster.serviceNameOfType(CmServiceType.YARN) : cluster
                     .serviceNameOfType(CmServiceType.MAPREDUCE)));
               break;
            case OOZIE:
               apiServiceConfig.add(new ApiConfig("mapreduce_yarn_service", serviceTypes
                     .contains(CmServiceType.YARN) ? cluster.serviceNameOfType(CmServiceType.YARN) : cluster
            ...
            case HIVE:
               ...
               if (versionApi >= 4) {
                  apiServiceConfig.add(new ApiConfig("zookeeper_service", cluster
                        .serviceNameOfType(CmServiceType.ZOOKEEPER)));
               }
               break;
            case IMPALA:
               apiServiceConfig.add(new ApiConfig("hdfs_service", cluster.serviceNameOfType(CmServiceType.HDFS)));
         ...
         apiResourceRootV3.getClustersResource().getServicesResource(cluster.getName()).createServices(serviceList);
      ...
      for (CmServiceType type : cluster.allServiceTypes()) {
         for (ApiCommand command : apiResourceRootV3.getClustersResource().getServicesResource(cluster.getName())
               .listActiveCommands(cluster.serviceNameOfType(type), DataView.SUMMARY)) {
            execute(command, false);
         }
      }

      execute(apiResourceRootV3.getClustersResource().deployClientConfig(cluster.getName()));
   }

   private String getName(CmClusterSpec cluster) {
      try {
         return cluster.getServiceName(CmServiceType.CLUSTER);
      } catch (IOException e) {
