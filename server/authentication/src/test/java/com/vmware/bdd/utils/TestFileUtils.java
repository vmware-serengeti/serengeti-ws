...
public class TestFileUtils {
   ...
   public static void createFile() {
      file = new File(TestFileUtils.TestFile);
      try {
         if (!file.exists()) {
            file.createNewFile();
         }
   ...
   public static void createXMLFile(Users users, File output) throws JAXBException {
      JAXBContext jaxbContext;
      jaxbContext = JAXBContext.newInstance(Users.class);
      Marshaller marshaller = jaxbContext.createMarshaller();
      marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
      marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
      marshaller.marshal(users, output);
   }

   public static void deleteXMLFile(File output) {
      if (output != null) {
         output.delete();
      }
   }
