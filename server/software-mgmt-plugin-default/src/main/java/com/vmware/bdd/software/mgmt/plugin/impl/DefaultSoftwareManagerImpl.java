...
public class DefaultSoftwareManagerImpl implements SoftwareManager {
   ...
   @Override
   public boolean validateConfiguration(ClusterBlueprint blueprint)
    ...
    public boolean validateRoles(ClusterBlueprint blueprint, List<String> roles) throws SoftwareManagementPluginException {
        ...
        //validateNodeGroupRoles(failedMsgList, cluster);

        if (!failedMsgList.isEmpty()) {
            SoftwareManagementPluginException e = new SoftwareManagementPluginException();
     ...
     *
     */
    private void checkClusterRoles(ClusterBlueprint blueprint, List<String> distroRoles, List<String> failedMsgList) {
            ...
            if (roles != null) {
                for (String role : roles) {
                    ...
                    if (!ChefServerUtils.isValidRole(role)) {
                        ...
                        rolesMsg.append("role ").append(role).append(" is not supported by distro ").append(blueprint.getHadoopStack().getDistro());
                    }
    ...
    */
    /*private boolean validateNodeGroupRoles(List<String> failedMsgList, ClusterCreate clusterCreate) {
        boolean valid = true;
        Set<String> roles = new HashSet<>();
        NodeGroupCreate[] nodeGroups = clusterCreate.getNodeGroups();
        if (nodeGroups == null) {
            return false;
        }

        for (NodeGroupCreate nodeGroupCreate : nodeGroups) {
...
*/
    private boolean hasHDFSUrlConfigured() {
        //TODO(qjin): implement
        //return getExternalHDFS() != null && !getExternalHDFS().isEmpty();
        return false;
    }

    private boolean validateHDFSUrl() {
    ...
    private String getExternalHDFS() {
        //TODO(qjin): implement
        return null;
    }
